...
async function handler(req: NextRequest): Promise<NextResponse<unknown>> {
  ...
  try {
    ...
    try {
      ...
      if (pmRes.ok) {
        const pmMarkets: Array<{ question: string; outcomePrices?: string }> =
          await pmRes.json();
        if (pmMarkets.length > 0) {
          polymarketData.question = pmMarkets[0].question;
          if (pmMarkets[0].outcomePrices) {
            const prices: string[] = JSON.parse(pmMarkets[0].outcomePrices);
